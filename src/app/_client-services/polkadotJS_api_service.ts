...
// Usage:
// const apiService = await PolkadotJSApiService.Init(ENetwork.POLKADOT);
// const blockHeight = await apiService.getBlockHeight();

export class PolkadotJSApiService {
	...
	static async Init(network: ENetwork): Promise<PolkadotJSApiService> {
		const api = await ApiPromise.create({
			provider: new WsProvider(NETWORKS_DETAILS[`${network}`].rpcEndpoints[0].url)
		});

		await api.isReady;

		return new PolkadotJSApiService(network, api);
	}
	...
	private async executeTx({
		...
		if (wallet === EWallet.MIMIR) {
			...
			const signer = new VaultQrSigner(setVaultQrState, this.api.registry);
	...
	getPreimageParams({ extrinsicFn }: { extrinsicFn: SubmittableExtrinsicFunction<'promise'> }) {
		if (!this.api) {
			return [];
		}

		const submittable = this.api.tx[`${extrinsicFn.section}`][`${extrinsicFn.method}`];
				...
				if (beneficiary.assetId && ValidatorService.isValidAssetId(beneficiary.assetId, this.network)) {
					tx.push(
						this.api.tx?.treasury?.spend(
							...
							[ENetwork.KUSAMA].includes(this.network)
	...
	async getReferendaInfo({ postId }: { postId: number }) {
		...
		const info = referendaInfo.toHuman() as { [key: string]: any[] };

		const canRefundDecisionDeposit = !!(info?.Approved?.[2] || info?.Cancelled?.[2] || info?.Rejected?.[2] || info?.TimedOut?.[2]);
		const canRefundSubmissionDeposit = !!(info?.Approved?.[1] || info?.Cancelled?.[1]);
	...
	async getProxyRequests({ page, limit, search }: { page: number; limit: number; search?: string }) {
		...
		try {
			...
			proxyEntries.forEach(([key, value]) => {
				...
				const proxyRequest = PolkadotJSApiService.processProxyInfo(delegator, proxyData);
	...
	async getMyProxies({ page, limit, userAddress }: { page: number; limit: number; search?: string; userAddress: string }) {
		...
		try {
			...
			const proxyRequest = PolkadotJSApiService.processProxyInfo(userAddress, proxyInfo);
			if (proxyRequest) proxies.push(proxyRequest);
