				...
				{usdtValue && (
					<div className='flex items-center gap-2'>
						...
						{usdtExternalLink && (
							...
							>
								Address #1 <HiOutlineExternalLink className='h-4 w-4 cursor-pointer' />
							</Link>
						)}
						{secondUsdtExternalLink && (
							<Link
								...
								rel='noopener noreferrer'
							>
								Address #2 <HiOutlineExternalLink className='h-4 w-4 cursor-pointer' />
							</Link>
						)}
					</div>
				)}
...
const formatTreasuryValues = (data: ITreasuryStats) => {
	...
	return {
		...
		// Special case for ambassador value (could be DOT or USDT)
...
function LoansSection({ data, treasury }: LoansSectionProps) {
	return (
		<div>
			<div className='flex items-center gap-2'>
				<span className='text-sm text-btn_secondary_text'>Loans</span>
	...
	return (
		...
		>
			<DialogContent className='bg-bg_modal p-4 lg:max-w-4xl lg:p-6'>
				...
				{!isDataLoaded ? (
					<div className='py-2 text-center'>Loading treasury data...</div>
				) : (
					<div>
						<h2 className='text-text_grey'>Across Networks:</h2>
