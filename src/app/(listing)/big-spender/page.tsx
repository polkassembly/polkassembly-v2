...
async function BigSpenderPage({ searchParams }: { searchParams: Promise<{ page?: string; trackStatus?: string }> }) {
	const page = parseInt((await searchParams).page || '1', 10);
	const statuses = (await searchParams).trackStatus === 'all' ? [] : (await searchParams).trackStatus?.split(',') || [];

	const { data, error } = await NextApiClientService.fetchListingDataApi(EProposalType.REFERENDUM_V2, page, statuses, [EPostOrigin.BIG_SPENDER], []);
	...
	return (
		<div>
			<ListingPage
				title='Big Spender'
				description='Proposals that can spend a large amount of treasury funds'
