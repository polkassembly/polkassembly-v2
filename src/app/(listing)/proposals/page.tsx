...
export async function generateMetadata(): Promise<Metadata> {
	...
	return getGeneratedContentMetadata({
		...
		description: 'Explore all Proposals on Polkassembly',
		url: `https://${network}.polkassembly.io/proposals`,
		imageAlt: 'Polkassembly Proposals',
		network
	});
}
...
async function ProposalsPage({ searchParams }: { searchParams: Promise<{ page?: string; trackStatus?: string }> }) {
	const searchParamsValue = await searchParams;
	const { page, status: statuses } = zodQuerySchema.parse(searchParamsValue);

	const { data, error } = await NextApiClientService.fetchListingData({ proposalType: EProposalType.DEMOCRACY_PROPOSAL, page, statuses });
	...
	return (
		<div>
			<ListingPage
				proposalType={EProposalType.DEMOCRACY_PROPOSAL}
