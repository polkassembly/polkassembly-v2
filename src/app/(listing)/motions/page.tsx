...
export async function generateMetadata(): Promise<Metadata> {
	...
	return getGeneratedContentMetadata({
		title: `${title} - Motions`,
		description: 'Explore all Motions on Polkassembly',
		url: `https://${network}.polkassembly.io/motions`,
		imageAlt: 'Polkassembly Motions',
		network
	});
}
...
async function MotionsPage({ searchParams }: { searchParams: Promise<{ page?: string; status?: string }> }) {
	const searchParamsValue = await searchParams;
	const { page, status: statuses } = zodQuerySchema.parse(searchParamsValue);

	const { data, error } = await NextApiClientService.fetchListingData({ proposalType: EProposalType.COUNCIL_MOTION, page, statuses });
	...
	return (
		<div>
			<ListingPage
				proposalType={EProposalType.COUNCIL_MOTION}
