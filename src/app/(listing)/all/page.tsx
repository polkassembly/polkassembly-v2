...
export async function generateMetadata(): Promise<Metadata> {
	...
	return getGeneratedContentMetadata({
		title: `${title} - All OpenGov Proposals`,
		description: 'Explore all OpenGov proposals on Polkassembly',
		url: `https://${network}.polkassembly.io/all`,
		imageAlt: 'Polkassembly All OpenGov Proposals',
		network
	});
}
...
async function AllProposalsPage({ searchParams }: { searchParams: Promise<{ page?: string; status?: string }> }) {
	const searchParamsValue = await searchParams;
	const { page, status: statuses } = zodQuerySchema.parse(searchParamsValue);

	const { data, error } = await NextApiClientService.fetchListingData({ proposalType: EProposalType.REFERENDUM_V2, page, statuses });
