...
async function TreasurerPage({ searchParams }: { searchParams: { page?: string; trackStatus?: string } }) {
	const page = parseInt(searchParams.page || '1', 10);
	const statuses = searchParams.trackStatus === 'all' ? [] : searchParams.trackStatus?.split(',') || [];

	const { data, error } = await NextApiClientService.fetchListingDataApi(EProposalType.REFERENDUM_V2, page, statuses, [EPostOrigin.TREASURER], []);
	...
	return (
		<div>
			<ListingPage
				title='Treasurer'
				description='This page shows all treasurer proposals'
