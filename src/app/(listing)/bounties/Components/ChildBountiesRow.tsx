		...
		<>
			{childBounties &&
				childBounties.items.map((childBounty: IPost, index: number) => {
					// Build composite index from onChainInfo for URL navigation
					...
					const compositeIndex = parentBountyIndex !== undefined ? buildCompositeIndex(parentBountyIndex, childIndex) : childIndex;

					return (
						<TableRow
							key={compositeIndex}
							className={`${styles.tableBodyRow} ${styles.childBountyRow}`}
							onClick={() => router.push(`/child-bounty/${compositeIndex}`)}
						>
							<TableCell className='p-6'>
								<div className='flex h-6 items-center justify-start space-x-4 pl-2 pt-1'>
									...
									<span className='ml-8'>#{childIndex}</span>
								</div>
							</TableCell>
							...
							<TableCell className={styles.tableCell}>
								{childBounty.onChainInfo?.reward
									? formatBnBalance(
											childBounty.onChainInfo.reward.toString(),
											{ withThousandDelimitor: false, withUnit: true, numberAfterComma: 2, compactNotation: true },
											network
										)
									: '-'}
							</TableCell>
