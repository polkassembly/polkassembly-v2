...
import { getVoteColor, getVoteIcon } from '@/_shared/_utils/dvVoteHelpers';
...
function VoteHeatmapRow({ item, referendumIndices }: { item: IDVDelegateVotingMatrix; referendumIndices: number[] }) {
	...
	const ayeCount = referendumIndices.filter((ref) => item.votes[ref] === EVoteDecision.AYE).length;
	const nayCount = referendumIndices.filter((ref) => item.votes[ref] === EVoteDecision.NAY).length;
	const abstainCount = referendumIndices.filter((ref) => item.votes[ref] === EVoteDecision.ABSTAIN).length;
	const totalCount = referendumIndices.length;

	const ayePercent = (ayeCount / totalCount) * 100;
	const nayPercent = (nayCount / totalCount) * 100;
	const abstainPercent = (abstainCount / totalCount) * 100;
		...
		<div className='w-full rounded-2xl border border-border_grey bg-bg_modal'>
									...
									<TooltipProvider>
										<Tooltip>
											<TooltipTrigger asChild>
												<div className='flex h-full w-full cursor-pointer flex-col items-center justify-center'>{getVoteIcon(vote)}</div>
											</TooltipTrigger>
			...
			<div className='w-full px-0'>
				<TooltipProvider>
					<Tooltip>
						<TooltipTrigger asChild>
							<div className='flex h-1.5 w-full cursor-pointer overflow-hidden rounded-b-2xl bg-activity_selected_tab'>
								{ayePercent > 0 && (
									<div
										className='h-full bg-social_green'
										style={{ width: `${ayePercent}%` }}
									/>
								)}
								{nayPercent > 0 && (
									<div
										className='h-full bg-failure'
										style={{ width: `${nayPercent}%` }}
									/>
								)}
								{abstainPercent > 0 && (
									<div
										className='h-full bg-dv_voting_card_abstain_bar_color'
						...
						<TooltipContent className='bg-tooltip_background text-btn_primary_text'>
							<p>
								{t('TotalVotes')}: {item.activeCount}/{totalCount}
							</p>
						</TooltipContent>
					</Tooltip>
				</TooltipProvider>
