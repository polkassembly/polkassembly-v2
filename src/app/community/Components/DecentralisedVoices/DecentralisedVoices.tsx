...
import { ECohortStatus, EDVDelegateType } from '@/_shared/types';
...
function DecentralisedVoices() {
	...
	const guardiansCount = cohort?.delegates.filter((d) => d.role === EDVDelegateType.GUARDIAN).length || 0;

	return (
		<div>
			<TabCard
				cohortNumber={cohortId}
				delegates={cohort?.delegates.length || 0}
