...
export const getAllReferenda = async (network: ENetwork) => {
	// TODO:use network as a base to fetch referenda
	const { data, error } = await request('/listing/on-chain-posts?proposalType=referendums_v2', {}, { method: 'POST' });
	if (error) {
		console.error(`Failed to fetch referenda for network ${network}:`, error);
		return null;
	}
	console.log(`Data for network ${network}:`, data);
	return data.posts.map(parseReferendum) as Array<IPost>;
};
