...
export const GET = withErrorHandling(
	async (req: NextRequest, { params }: { params: Promise<{ proposalType: string; index: string; id: string; reactionId: string }> }): Promise<NextResponse> => {
		const { reactionId } = zodParamsSchema.parse(await params);

		const reaction = await OffChainDbService.GetReactionById(reactionId);
...
export const DELETE = withErrorHandling(
	async (req: NextRequest, { params }: { params: Promise<{ proposalType: string; index: string; id: string; reactionId: string }> }): Promise<NextResponse> => {
		const { proposalType, index, reactionId } = zodParamsSchema.parse(await params);
		...
		await OffChainDbService.DeleteReactionById({ id: reactionId, userId });
