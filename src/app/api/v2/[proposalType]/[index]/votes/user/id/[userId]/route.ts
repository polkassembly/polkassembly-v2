...
const zodParamsSchema = z.object({
	...
	index: z.string(),
	userId: z.coerce.number().refine((val) => ValidatorService.isValidUserId(val), { message: 'Please enter a valid user ID' })
});

export const GET = withErrorHandling(async (req: NextRequest, { params }: { params: Promise<{ proposalType: string; index: string; userId: string }> }): Promise<NextResponse> => {
	const { proposalType, index, userId } = zodParamsSchema.parse(await params);
	...
	const userAddresses = await OffChainDbService.GetAddressesForUserId(userId);

	const voteData = await Promise.all(
		userAddresses.map(async (address) => {
			return OnChainDbService.GetPostVoteData({ network, proposalType, indexOrHash: index, voterAddress: address.address, page, limit, decision });
		})
	);
