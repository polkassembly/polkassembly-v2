...
export async function GET(req: NextRequest) {
	try {
		...
		if (!skipCache) {
			const cachedData = await RedisService.GetGovAnalyticsStats(network);
			if (cachedData) {
		...
		const data = await SubsquidService.GetGovAnalyticsStats({ network });
		...
		console.error('Error in GET /api/v2/gov-analytics/stats:', error);
		return Response.json({ error: 'Failed to fetch network governance analytics stats' }, { status: 500 });
	}
}
