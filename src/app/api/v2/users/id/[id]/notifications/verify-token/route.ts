...
export const POST = withErrorHandling(async (request: NextRequest, { params }: IRouteParams): Promise<NextResponse> => {
	...
	const { channel, token, handle } = (await request.json()) as {
		channel: ENotificationChannel;
		token: string;
		handle: string;
	};

	if (!channel || !token || !handle) {
		throw new APIError(ERROR_CODES.INVALID_PARAMS_ERROR, StatusCodes.BAD_REQUEST, 'Missing required parameters');
	}

	if (!Object.values(ENotificationChannel).includes(channel)) {
		throw new APIError(ERROR_CODES.INVALID_PARAMS_ERROR, StatusCodes.BAD_REQUEST, 'Invalid notification channel');
	}

	const isVerified = await NotificationPreferencesService.VerifyChannelToken(userId, channel, token, handle);

	if (!isVerified) {
		throw new APIError(ERROR_CODES.INVALID_PARAMS_ERROR, StatusCodes.BAD_REQUEST, 'Invalid or expired token');
	}
