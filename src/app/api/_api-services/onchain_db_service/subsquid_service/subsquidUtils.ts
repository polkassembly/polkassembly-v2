...
import { ENetwork, EPostOrigin, EProposalStatus, EVoteDecision, IBeneficiary, IVoteCurve, IVoteMetrics, IVoteProgress } from '@/_shared/types';
...
export class SubsquidUtils extends SubsquidQueries {
	...
	protected static getAllPeriodEndDates(statusHistory: IStatusHistory[], network: ENetwork, origin: EPostOrigin): IPeriodEndDates {
		...
		try {
			...
			const decidingStatus = statusHistory.find((status) => DECIDING_PROPOSAL_STATUSES.includes(status.status));
	...
	protected static async calculateVoteMetricsAndProgress({
		network,
		proposal,
		voteMetrics
	}: {
