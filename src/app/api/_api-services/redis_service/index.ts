...
import { ENetwork, IDelegate, IDelegationStats, IGenericListingResponse, IPost, IPostListing } from '@/_shared/types';
...
export class RedisService {
	...
	private static readonly redisKeysMap = {
		...
		[ERedisKeys.DELEGATES]: (network: string): string => `${ERedisKeys.DELEGATES}-${network}`,
	...
	static async GetDelegates(network: ENetwork): Promise<IDelegate[] | null> {
		const data = await this.Get({ key: this.redisKeysMap[ERedisKeys.DELEGATES](network), forceCache: true });
		return data ? (deepParseJson(data) as IDelegate[]) : null;
	}

	static async SetDelegates(network: ENetwork, data: IDelegate[]): Promise<void> {
		await this.Set({ key: this.redisKeysMap[ERedisKeys.DELEGATES](network), value: JSON.stringify(data), ttlSeconds: ONE_DAY });
	}

	static async GetDelegationStats(network: ENetwork): Promise<IDelegationStats | null> {
		const data = await this.Get({ key: this.redisKeysMap[ERedisKeys.DELEGATION_STATS](network), forceCache: true });
