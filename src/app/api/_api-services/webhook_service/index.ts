...
import { EAnalyticsType, EHttpHeaderKey, ENetwork, EVotesType, EPostOrigin, EProposalType, ERole, EWallet, IUser } from '@/_shared/types';
...
export class WebhookService {
	...
	private static async handleVoted({ network, params }: { network: ENetwork; params: z.infer<(typeof WebhookService.zodEventBodySchemas)[EWebhookEvent.VOTED]> }) {
		...
		await Promise.all([
			...
			RedisService.DeletePostBubbleVotesData({ network, proposalType, index: Number(indexOrHash), votesType: EVotesType.FLATTENED, analyticsType: EAnalyticsType.VOTES }),
			RedisService.DeletePostBubbleVotesData({ network, proposalType, index: Number(indexOrHash), votesType: EVotesType.NESTED, analyticsType: EAnalyticsType.VOTES })
		]);
	}
