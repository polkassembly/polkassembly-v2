...
export class PriceService {
	...
	private static async getCoinGeckoPrice(symbol: string): Promise<IPriceResponse | null> {
		try {
			const response = await fetch(`${this.COIN_GECKO_API}?ids=${symbol}&vs_currencies=usd`);
	...
	private static async getKrakenPrice(symbol: string): Promise<IPriceResponse | null> {
		try {
			const response = await fetch(`${this.KRAKEN_API}?pair=${symbol}USD`);
			const data = await response.json();
	...
	private static async getHuobiPrice(symbol: string): Promise<IPriceResponse | null> {
		try {
			const response = await fetch(`${this.HUOBI_API}?symbol=${symbol.toLowerCase()}usdt`);
			const data = await response.json();
			if (data?.tick?.close) {
				return {
	...
	private static async getKucoinPrice(symbol: string): Promise<IPriceResponse | null> {
		try {
			const response = await fetch(`${this.KUCOIN_API}?symbol=${symbol}-USDT`);
			const data = await response.json();
			if (data?.data?.price) {
				return {
