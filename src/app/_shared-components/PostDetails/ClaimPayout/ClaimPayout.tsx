...
function ClaimPayout({ beneficiaries }: { beneficiaries: IBeneficiary[] }) {
	...
	const fetchPendingTreasurySpends = async () => {
		...
		const relayChainBlockHeight = [ENetwork.KUSAMA, ENetwork.POLKADOT].includes(network) ? await assethubApiService?.getBlockHeight() : await apiService?.getBlockHeight();
