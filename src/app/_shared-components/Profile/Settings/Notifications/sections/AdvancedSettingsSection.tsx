...
import AdvancedSettingsIcon from '@assets/icons/advancedsettings.svg';
...
function AdvancedSettingsSection() {
	...
	const { preferences, updateOpenGovTrack, updateGov1Item, bulkUpdateAdvancedSettings, bulkUpdateTrackNotifications } = useNotificationPreferences();

	const openGovTracks = preferences?.openGovTracks || {};
	const gov1Items = preferences?.gov1Items || {};

	const handleOpenGovTrackChange = (trackKey: string, enabled: boolean) => {
		// Use bulk update function for individual tracks
		bulkUpdateTrackNotifications(trackKey, enabled, 'opengov');
	};
	...
	const handleGov1ItemChange = (itemKey: string, enabled: boolean) => {
		...
		bulkUpdateTrackNotifications(itemKey, enabled, 'gov1');
	};
	...
	// Check if all tracks have enabled: true (not just existence)
	...
	const toggleAllAdvanced = () => {
		...
		bulkUpdateAdvancedSettings(newState);
	};
