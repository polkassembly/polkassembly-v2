...
function BountiesNotificationsSection() {
	const { preferences, updateBountiesNotification, bulkUpdateBountiesNotifications } = useNotificationPreferences();

	const bountiesNotifications = preferences?.bountiesNotifications || {
	...
	const handleBountiesNotificationChange = (type: string, enabled: boolean) => {
		...
		updateBountiesNotification(type, {
			...currentSettings,
			enabled
		});
	};
	...
	const toggleAllBounties = () => {
		...
		bulkUpdateBountiesNotifications(newState);
	};
