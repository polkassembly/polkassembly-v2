...
const useHydrationApi = (network: string) => {
	...
	useEffect(() => {
		const initApi = async () => {
			try {
				const wsProvider = new WsProvider(NETWORKS_DETAILS?.[ENetwork.ROCOCO]?.hydrationEndpoints?.[0]);
	...
	const fetchHydrationAssetsAmount = async () => {
		...
		const { hydrationTreasuryAddress, hydrationTreasuryAddress2, hydrationAssets } = NETWORKS_DETAILS?.[ENetwork.ROCOCO] || {};
